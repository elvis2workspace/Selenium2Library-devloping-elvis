...
ROBOT_ARGS = [
    '--selenium2library_doc', 'SeleniumSPacceptanceSPtestsSPwithSP%(browser)s',
    '--outputdir', '%(outdir)s',
...
def execute_tests(runner, args):
    ...
    print('')
    print('Starting test_selenium2library execution with command:\n' + ' '.join(command))
...
def process_output(args):
    ...
    if rc == 0:
        ...
        print('%d critical test_selenium2library%s failed' % (rc, 's' if rc != 1 else ''))
    return rc
