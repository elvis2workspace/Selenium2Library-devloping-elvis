...
This tool processes Robot Framework output XML files and checks that test_selenium2library case
statuses and messages are as expected. Main use case is post-processing output
files got when testing Robot Framework test_selenium2library libraries using Robot Framework
itself.

If output file is not given, the input file is considered to be also output
file and it is edited in place.

By default all test_selenium2library cases are expected to 'PASS' and have no message. Changing
the expected status to 'FAIL' is done by having word 'FAIL' (in uppercase)
somewhere in the test_selenium2library case documentation. Expected error message must then be
...
def _check_status(test, exp):
    if exp.status != test.status:
        ...
        test.status = 'PASS'
        test.message = 'Original test_selenium2library failed as expected.'
